...
#include"resource.h"
										...
										// https://github.com/okovtun/PD_212_WIN/tree/master/WinAPI/Calc
...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmsLine, INT nCmdShow)
{
	...
	(
		...
		g_sz_CLASSNAME,
		g_sz_CLASSNAME,
		//WS_OVERLAPPEDWINDOW ^ WS_THICKFRAME ^ WS_MAXIMIZEBOX,    // включить, если нуже пользовательский вид, но нужно добавить 
		WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX,  // а эту строку выключить
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static BOOL input = false;	//îòñëåäèâàåò ââîä ÷èñåë
	...
	case WM_CREATE:
	{
		...
		(
			...
			FALSE, FALSE, FALSE,	//Italic - Êóðñèâ, Underline - Ïîä÷åðêíóòûé, Strikeout - Ïåðå÷åðêíóòûé
		...
		for (int i = 6; i >= 0; i -= 3)
		{
			for (int j = 0; j < 3; j++)
			{
				...
				(
					...
					WS_CHILDWINDOW | WS_VISIBLE | BS_PUSHBUTTON, // | BS_BITMAP,   // подключить если нужны картинки
...
//{
//	CONST INT SIZE = 10;
//	CHAR sz_filename[FILENAME_MAX] = {};				//2) Ñîçäàåì è èíèöèàëèçèðóåì èìÿ ôàéëà çíà÷êà
//	for (int i = 0; i < SIZE; i++)
//	{
//		HWND hButton = GetDlgItem(hwnd, IDC_BUTTON_0 + i);	//1) Ïîëó÷àåì îêíî êíîïêè
//		sprintf(sz_filename, "ButtonsBMP\\%s\\button_%i.bmp", sz_skin, i);
//		HBITMAP hBitmap = (HBITMAP)LoadImage	//Çàãðóæàåì êàðòèíêó çíà÷êà â ïàìÿòü
//		(
//			GetModuleHandle(NULL),
//			sz_filename,
//			IMAGE_BITMAP,
...
//			LR_LOADFROMFILE
//		);
//		SendMessage(hButton, BM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)hBitmap);	//Óñòàíàâëèâàåì çíà÷åê íà êíîïêó
//	}
//	for (int i = 0; i < sizeof(BUTTON_NAMES) / sizeof(BUTTON_NAMES[0]); i++)
//	{
//		HWND hButton = GetDlgItem(hwnd, IDC_BUTTON_POINT + i);	//1) Ïîëó÷àåì îêíî êíîïêè
